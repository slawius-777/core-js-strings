...
function removeTrailingWhitespaces(value) {
  if (typeof value !== 'string') {
    throw new Error('imput must be a string');
  }
...
function repeatString(str, times) {
  // Check if times is a non-negative integer
  if (times < 0 || !Number.isInteger(times)) {
    return ''; // Return an empty string if the number is negative or not an integer
  }
  return str.repeat(times); // Repeat the string the specified number of times
}
...
function removeFirstOccurrences(str, value) {
  const index = str.indexOf(value); // Find the index of the first occurrence
  if (index === -1) return str; // If not found, return the original string
  return str.slice(0, index) + str.slice(index + value.length); // Remove the first occurrence
}
...
function removeLastOccurrences(str, value) {
  const index = str.lastIndexOf(value); // Find the last occurrence index
  if (index === -1) return str; // If not found, return the original string
  return str.slice(0, index) + str.slice(index + value.length); // Remove the last occurrence
}
...
function formatTime(minutes, seconds) {
  // Ensure minutes and seconds are two digits by padding with leading zeros
...
function invertCase(str) {
  return str
    ...
    .map((char) => {
      // Check if the character is uppercase or lowercase and invert
...
function getCardId(value) {
  ...
  const rank = value.slice(0, -1); // Extract the rank (e.g., 'A' from 'A♣')
  const suit = value.slice(-1); // Extract the suit (e.g., '♣' from 'A♣')

  const rankIndex = ranks.indexOf(rank);
  const suitIndex = suits.indexOf(suit);

  // Calculate the card's zero-based index
